...
const AddProductModal = ({ onClose, onAdd, categories }: { onClose: () => void, onAdd: (data: any) => void, categories: ProductCategory[] }) => {
  const [form, setForm] = useState<{ name: string; price: string; categoryId: string; active: boolean; image: File | null }>({ name: '', price: '', categoryId: '', active: true, image: null });
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    if (e.target.name === 'image') {
      ...
      setForm({ ...form, image: input.files?.[0] || null });
    } else {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onAdd(formData); // agora envia FormData para o backend
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-8 w-full max-w-md shadow-lg">
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <input name="image" type="file" accept="image/"  onChange={handleChange} className="w-full px-3 py-2 border rounded"/>
